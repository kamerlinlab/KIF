...
class PyContactInitializer():
    ...
    def _reformat_pycontact_file(self, input_file):
        ...
        with open(file_in_path, 'r') as f: 
            ...
            filedata = filedata.replace("hbond", "hbond,").replace("hydrophobic", "hydrophobic,").replace("other", "vdW,").replace("saltbr", "saltbr,")
    ...
    def _standardize_interaction_names(self, df):
        ...
        df_cols["Res1_num"] = df_cols["Val_1"].str.split("[a-zA-Z]+").str.get(1)
        df_cols["Res2_num"] = df_cols["Val_2"].str.split("[a-zA-Z]+").str.get(1)
