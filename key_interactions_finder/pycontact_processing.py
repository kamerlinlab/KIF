...
1. A vdw interaction between a residue to itself.
2. Duplicate interactions with the only difference being residue ordering swapped.

If a job is run in the above way the 'remove_false_interactions' must be set to True
(default) when the class is initialised in order to remove these false interactions.
No harm if job not run in this way but 'remove_false_interactions' is True.
"""
import re
...
class PyContactInitializer():
    ...
    def _interaction_is_duplicate(self, contact_parts: list, contacts_to_keep: list) -> bool:
        """
    ...
    def _rm_false_interactions(self) -> pd.DataFrame:
        """
